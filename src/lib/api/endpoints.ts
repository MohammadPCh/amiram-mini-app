...
export const be = {
  ...
  rewards: {
    list: (params: { page?: number; status?: string }) => {
      ...
      return apiFetch<RewardsListResponse>(`/api/rewards?page=${page}&status=${encodeURIComponent(status)}`)
    },
    claim: (rewardId: number) =>
      apiFetch<RewardClaimResponse>(`/api/rewards/${rewardId}/claim`, { method: 'POST' }),
    checkout: (req: CheckoutRequest) =>
      apiFetch<CheckoutResponse>('/api/rewards/checkout', { method: 'POST', body: JSON.stringify(req) }),
  },

  wheel: {
    config: () => apiFetch<WheelConfigResponse>('/api/wheel/config'),
    spin: () => apiFetch<WheelSpinResponse>('/api/wheel/spin', { method: 'POST' }),
  },
